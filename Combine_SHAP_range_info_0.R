...
for(i in 1:length(vars))
{
  ...
  #save plot image
  setwd(wd_res)
  pdf(file = paste0(vars[i],'_Absolute_Polarwardness.pdf'))
  ...
  #fit linear model
  lin_mod <- lm(cont_var ~ sps_var_SHAP_info2$relPolarwardness)
  abline(lin_mod, col = 'red', lwd = 2)
  ...
  #save plot image
  setwd(wd_res)
  pdf(file = paste0(vars[i],'_Relative_Polarwardness.pdf'))
  
  #set parametres for plotting
  par(mar = c(4,4,4,4))
  
  ### Variable contribution per centralness
  plot(sps_var_SHAP_info2$centralness, cont_var, 
       pch = 19, cex = 0.4, col = 'gray80',
       ylab = paste0('Contribution ', var),
       xlab = 'Centralness')
  
  #fit linear model
  lin_mod <- lm(cont_var ~ sps_var_SHAP_info2$centralness)
  abline(lin_mod, col = 'red', lwd = 2)
