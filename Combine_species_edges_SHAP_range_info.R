...
wd_res <- '/Users/carloseduardoaribeiro/Documents/Post-doc/SHAP/Mammals/Results/Results_analyses/Each_species_edge'
...
for(j in 1:length(sps_list_sel))
{
  ...
  if(j != 11 & j!= 28){
    ...
    #setwd(wd_test)
    setwd(wd_sps)
    pdf(file = paste0(gsub(' ', '_', sps_list_sel[j]),'_Temperature_Edge_RelPolar.pdf'))
    ...
    pdf(file = paste0(gsub(' ', '_',
                           sps_list_sel[j]),'_Temperature_Edge_Centralness.pdf'))
...
temp_edge <- data.frame(species = species, 
                           minTR2_RP = minTR2_RP,
...
#save table
setwd(wd_tables)
write.csv(temp_edge, 'Temperature_edge_points.csv', row.names = F)
...
for(j in 1:length(sps_list_sel))
{
  ...
  if(j != 11 & j!= 28){
    ...
    pdf(file = paste0(gsub(' ', '_', sps_list_sel[j]),
                      '_Precipitation_Edge_RelPolar.pdf'))
    ...
    setwd(wd_sps)
    pdf(file = paste0(gsub(' ', '_',
                           sps_list_sel[j]),'_Precipitation_Edge_Centralness.pdf'))
...
#create a dataframe with the results for all species
prec_edge <- data.frame(species = species, 
...
#save table
setwd(wd_tables)
write.csv(prec_edge, 'Precipitation_edge_points.csv', row.names = F)
