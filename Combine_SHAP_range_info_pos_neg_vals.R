...
wd_res <- '/Users/carloseduardoaribeiro/Documents/Post-doc/SHAP/Mammals/Results/Results_analyses/All_species_all_points_posneg'
...
## Create vector with variable contribution (considering positive or negative)

#minT
cont_minT <- sps_minT_SHAP_info_allSpp[,9]

#meanT
cont_meanT <- sps_meanT_SHAP_info_allSpp[,9]

#maxT
cont_maxT <- sps_maxT_SHAP_info_allSpp[,9]



## Save plot image temperature variables contribution per absolute polarwardness
setwd(wd_res)
pdf(file = 'AllSpp_Temperature_AbsPolar_Shap_non_mornamlised.pdf')
...
xlim <- range(sps_minT_SHAP_info_allSpp$absPolarwardness) #does not matter which I use
...
text(xlim[1], ylim[1] + (ylim[2] - ylim[1]) / 16, 
     expression(bold(paste('R'^2*' = '), sep='')), col = '#800080', pos = 4)
text(xlim[1] + 0.07, ylim[1] + (ylim[2] - ylim[1]) / 16,
...
text(xlim[1], ylim[1] + (ylim[2] - ylim[1]) / 8, 
     expression(bold(paste('R'^2*' = '), sep='')), col = '#FF0000', pos = 4)
text(xlim[1] + 0.07, ylim[1] + (ylim[2] - ylim[1]) / 8,
...
text(xlim[1], ylim[1] + (ylim[2] - ylim[1]) / 16, 
     expression(bold(paste('R'^2*' = '), sep='')), col = '#800080', pos = 4)
text(xlim[1] + 0.15, ylim[1] + (ylim[2] - ylim[1]) / 16,
     round(r2,3), col = '#800080', font = 2, pos = 4)
...
text(xlim[1], ylim[1] + (ylim[2] - ylim[1]) / 8, 
     expression(bold(paste('R'^2*' = '), sep='')), col = '#FF0000', pos = 4)
text(xlim[1] + 0.15, ylim[1] + (ylim[2] - ylim[1]) / 8,
     round(r2,3), col = '#FF0000', font = 2, pos = 4)
...
#minT
cont_minPPT <- sps_minPPT_SHAP_info_allSpp[,9]

#meanT
cont_meanPPT <- sps_meanPPT_SHAP_info_allSpp[,9]

#maxT
cont_maxPPT <- sps_maxPPT_SHAP_info_allSpp[,9]
...
text(xlim[1], ylim[1], 
     expression(bold(paste('R'^2*' = '), sep='')), col = '#ff8d59', pos = 4)
text(xlim[1] + 0.07,
     ylim[1], 
     round(r2,3), col = '#ff8d59', font = 2, pos = 4)
...
text(xlim[1] + 0.15,
     ylim[1], round(r2,3), col = '#fc8d59', font = 2, pos = 4)
