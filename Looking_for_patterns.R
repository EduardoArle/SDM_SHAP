...
res_temp2 <- res_temp[order(res_temp$minTslope_RP),] #order for the plot
...
#add points minT
points(res_temp2$minTslope_RP, c(1:nrow(res_temp2)), pch = 19,
       col = 'blue',
       cex = res_temp2$latAmplitude_bin)


plot(abs(res_temp2$minTslope_RP), res_temp2$latAmplitude)

#########


t <- data.frame(minTslope_RP = res_temp$minTslope_RP,
                minTSE_RP = res_temp$minTSE_RP,
                one_SE_x_slope_minT_RP =res_temp$one_SE_x_slope_minT_RP)
